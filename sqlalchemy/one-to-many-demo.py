...
class Breed(Base):
    ...
    pets = relationship("Pet", backref="breed")  # added to refer to the pets table
...
class Pet(Base):
    ...
    has a with many-to-one relationship Breed
    May or may not have  shelter
    """
    __tablename__ = 'pet'
    ...
    name  = Column(String, nullable = False)
    age = Column(Integer)
    adopted = Column(Integer)   # how to make boolean??
...
if __name__ == "__main__":
    ...
    log.info("Now creating two new shelters")
    NYC_Pet_Orphanage = Shelter(name = "NYC Pet Orphanage", 
        website = "http://www.nyc-pet-orphanage.com")
    New_Orleans_Pet_Hotel = Shelter(name = "New Orleans Pet Hotel", 
        website = "http://www.neworleanspethotel.com")

    log.info("Now adding shelters to the the session and committing")
    db_session.add_all([NYC_Pet_Orphanage, New_Orleans_Pet_Hotel])
    log.info("There are now {} shelters in the database".format(
        db_session.query(Shelter).count() ) )
    ...
    log.info("Now creating two new pets")

    # Thomas = Pet(name = "Thomas", age = 5, adopted = 1, 
    #     breed_id = db_session.query(Breed).filter(Breed.name == "Golden Retriever"))

    Thomas = Pet(name = "Thomas", age = 5, adopted = 1, 
        breed_id = golden.id, shelter_id = NYC_Pet_Orphanage.id)
    Sue = Pet(name = "Sue", age = 8, adopted = 0, breed_id = poodle.id)
    log.info("Now adding the new pets ot the session and committing")
    db_session.add_all([Thomas, Sue])

    log.info("There are now {} pets, {} breeds, and {} species in the database".format(
        db_session.query(Pet).count(), db_session.query(Breed).count(),
